...
echo $sql = "SELECT * FROM tb_complaints WHERE ".$where_clause."";
	$row = mysql_query($sql);
	$compliantcount = mysql_num_rows($row);
	
	//echo $compliantcount;
	
echo $sql1 = "SELECT * FROM tb_complaints WHERE ".$where_clause." AND `status` = 'Closed' " ;
	$row = mysql_query($sql1);
	$closedcompliants = mysql_fetch_object($row);	
...
echo $sp  = "select sum(price) as sumprice from tb_complaints where ".$where_clause." ";
$res = mysql_query($sp);
$fetch = mysql_fetch_object($res);	
	
echo $cuser  = "select count(DISTINCT(user_id)) as totalusers from tb_complaints where ".$where_clause." ";
...
elseif($_GET['cid']!='' && $_GET['fromdate']!='' && $_GET['todate']!='')
{?>
...
<?php } else{?>
    ...
    <p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>">Total Posted Cases: <?php echo $getallresults;?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory.php?cid=<?php echo $_GET['cid']?>&status=Closed">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?></p>
