...
$eventimage = $callConfig->freeimageUploadcomncode("product",'image',"uploads/product/","uploads/product/thumbs/",$post['hdn_image'],'401','235');
	...
	function geteventDatacount($id)
  {
   ...
   $query=$callConfig->selectQuery(TPREFIX.TBL_PRODUCT_IMAGE,'*','c_id='.$id,'','','');
	...
	$fieldnames=array('complaint_category'=>$post['category'],'department'=>$post['department'],'country_id'=>$post['country'],'image'=>$bannerpic,'description'=>$post['description'],'status'=>'Open','user_id'=>$_SESSION['id'],'date_added'=>$createdOn,'price'=>$post['amount'],'title'=>$post['title']);
	//print_r($fieldnames);exit;
	 echo $res=$callConfig->updateRecord(TPREFIX.TBL_COMPLAINTS,$fieldnames,'id',$post['jama']);
	if($res!='')
	{
	header("location:".SITEURL."/viewcomplaints.php?id=".$_GET['id']."&msg=".$_GET['msg']."&err='Complaint updated successfully'" );
	}
	else{
	header("location:".SITEURL."/viewcomplaints.php?id=".$_GET['id']."&msg=".$_GET['msg']."&ferr='fail'" );
	}	
...
$whr="status='active'";
$query=$callConfig->selectQuery(TPREFIX.TBL_CATEGORY,'*',$whr,$order,0,7);
...
$fieldnames=array('complaint_category'=>$post['category'],'department'=>$post['department'],'subdept_id'=>$post['subdepartment'],'title'=>$post['title'],'country_id'=>$post['country'],'price'=>$post['paid'],'description'=>$post['description'],'status'=>'Open','user_id'=>$_SESSION['id'],'date_added'=>$post['posteddate']);
	...
	$createdOn= date("Y-m-d H:i:s");
	 $bannerpic = $callConfig->freeimageUploadcomncode("file",'image',"uploads/complaints/","uploads/complaints/thumbs/",$post['hdn_image'],'434','411'); /*'250','150*/
	$fieldnames=array('complaint_category'=>$post['category'],'department'=>$post['department'],'subdept_id'=>$post['subdepartment'],'title'=>$post['title'],'country_id'=>$post['country'],'price'=>$post['paid'],'image'=>$bannerpic,'description'=>$post['description'],'status'=>'Open','user_id'=>$_SESSION['id'],'date_added'=>$createdOn);
	//print_r($fieldnames);exit;
	$res=$callConfig->insertRecord(TPREFIX.TBL_COMPLAINTS,$fieldnames);
	if($res!='')
	{
	header("location:".SITEURL."/viewcomplaints.php?id=".$_GET['id']."&msg=".$_GET['msg']."&err='Complaint Registered successfully'" );
	}
	else{
...
function getallcomplaints()
  ...
  function getallcomplaintsnew()
  {
	 global $callConfig;
	   $whr = " complaint_category='".$_GET['id']."' and country_id='".$_GET['cont']."'";
	 $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	return $callConfig->getAllRows($query);
 ...
 function getnews($id)
  {
   ...
   function getallsubcat($id)	
	...
	{
		global $callConfig;
		$whr="complaint_category=".$igd." and status='Open'";
	  $query	= $callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','',''); 
		 return $callConfig->getAllRows($query);
	}
...
return $callConfig->getRow($query);
	...
	function selectdatetosearch($post)
  {
	 ...
	 header("location:".SITEURL."/dashboard6.php?cid=".$_POST['cid']."&sid=".$_POST['sid']."&fromdate=".$_POST['fromdate']."&todate=".$_POST['todate']." " );
	 	} else {
		header("location:".SITEURL."/dashboard6.php?cid=".$_POST['cid']."&fromdate=".$_POST['fromdate']."&todate=".$_POST['todate']." " );
...
return $callConfig->getAllRows($query);
	...
	function getcomplaintsbycountry($get)
	
	{
	
		global $callConfig;
$whr="complaint_category=".$get['cid']." and department=".$get['sid']." and country_id=".$_GET['countid']." ";
$cid="complaint_category=".$get['cid']."";
$sid="department=".$get['sid']."";
$count_id="country_id=".$get['count_id']."";
...
return $callConfig->getAllRows($query);
	...
	function getclosedcomplaintsbycountry($get)
	
	{
	
		global $callConfig;
$whr="status='Closed' and complaint_category=".$get['cid']." and department=".$_GET['sid']." and country_id=".$_GET['countid']." ";
$cid="complaint_category=".$get['cid']."";
$sid="department=".$get['sid']."";
$count_id="country_id=".$get['count_id']."";
...
return $callConfig->getAllRows($query);
	...
	function getcomplaintsbycountry1($get)
	
	{
	
		global $callConfig;
$whr="complaint_category=".$get['cid']." and country_id=".$_GET['countid']." ";
...
return $callConfig->getAllRows($query);
	...
	function getcomplaintsbycountry2($get)
	
	{
	
		global $callConfig;
$whr="status='Active' and complaint_category=".$get['cid']." and department=".$get['sid']." and country_id=".$get['countid']." ";
$cid="complaint_category=".$get['cid']."";
$sid="department=".$get['sid']."";
$count_id="country_id=".$get['count_id']."";
...
return $callConfig->getAllRows($query);
	...
	function getcomplaintsbycountry3($get)
...
$whr="status='Active' and complaint_category=".$get['cid']." and department=".$get['sid']." and country_id=".$get['countid']." and date_added between '".$fromdate."' and '".$todate."'";
...
return $callConfig->getAllRows($query);
	...
	function getclosedcomplaintsbyid($get)
	{
	
		global $callConfig;
$whr="status='Closed' and complaint_category=".$get['cid']." and  country_id=".$_GET['countid']." ";
...
return $callConfig->getAllRows($query);
	...
	function getallcomplaintsofcountry($get)
	{
	
		global $callConfig;
$whr="complaint_category=".$get['cid']." and  country_id=".$get['countid']." ";
$cid="complaint_category=".$get['cid']."";
$sid="department=".$get['sid']."";
$count_id="country_id=".$get['count_id']."";
...
return $callConfig->getAllRows($query);
	...
	//pavani queries for dashboard country
	
	
	function getcountrycomplaintsfordashboard($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']." and country_id=".$_GET['countid']." ";
	 $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	
	function getopencountrycomplaints($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']." and country_id=".$_GET['countid']." and status='Open' ";
	  $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	
	function getclosedcountrycomplaints($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']." and country_id=".$_GET['countid']." and status='Closed' ";
	 $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	...
	function getcountrycomplaintsfordashsub($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']." and department=".$_GET['sid']." and country_id=".$_GET['countid']." ";
	$query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	
	function getopencountrycomplaintsub($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']." and department=".$_GET['sid']." and country_id=".$_GET['countid']." and status='Open' ";
	  $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	
	function getclosedcountrycomplaintsub($get){
		
		global $callConfig;
	$whr= "complaint_category=".$_GET['cid']." and department=".$_GET['sid']." and country_id=".$_GET['countid']." and status='Closed' ";
	  $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
	...
	function getclosedcountrycomplaintsbycategory($get){
	...
	function getopencomplaintsbycategory($get){
		
		global $callConfig;
		$whr= "complaint_category=".$_GET['cid']."  and status='Open' ";
	 $query=$callConfig->selectQuery(TPREFIX.TBL_COMPLAINTS,'*',$whr,'','','');
	 //print_r($query);exit;
	 return $callConfig->getAllRows($query);
	}
...
function getallcatsbylimit(){
	...
	$password1=$callConfig->passwordEncrypt($_POST['password']);
		...
		'username'=>$username,
		'firstname'=>mysql_real_escape_string($_POST['fname']),
		'lastname'=>mysql_real_escape_string($post['lname']),
		'address'=>$post['address'],
		'email'=>$post['email'],
		'password'=>$password1,
		'phone_number'=>$post['phone'],		
	...
	//print_r($fieldnames); exit;
	 ...
	 $query=$callConfig->insertRecord(TPREFIX.TBL_REGUSERS,$fieldnames);
	...
	// print_r($row);
	 ...
	 if($query!='') {
		 ...
		 $where="email='".$_POST['email']."' and status='Inactive'";
	  $query2=$callConfig->selectQuery(TPREFIX.TBL_REGUSERS,'*',$where,'id DESC','','');  
	 $row=$callConfig->getRow($query2);
	...
	// print_r($row);exit;
		...
		//print_r($_POST);exit;
					$to=$post['email'];
					$subject = "Dynamic Database | Registration Details";
				    $message='<meta charset="utf-8">
								<meta name="viewport" content="width=device-width, initial-scale=1">

									<link href="../css/mail.css" rel="stylesheet" type="text/css">
		<link href="https://fonts.googleapis.com/css?family=Raleway:300,400" rel="stylesheet"> <div class="emailtemplateblg">
	<div class="emailcontentblg">
    	<div class="emaildesignlogoblg">
        	<a href="#"><img src="images/logo.png" alt=""></a>
        </div><!--emaildesignlogoblg-->
    	<div class="emaillogoblg">
        	
            <div class="emailtext">
             <p class="text1">Dear '.$_POST['fname'].' '.$_POST['lname'].',</p>
              <p class="text2">Please find registration details of yours</p>
              <p class="text3"><span>Email:</span><a href="#">'.$post['email'].'</a></p>
              <p class="text4"><span>Password:</span>'.$post['password'].'</p>
    <p class="text5"><span>Link: </span><a href='.SITEURL.'/login.php?rid='.$row->random_id.'>Click here to activate</a></p>
              <p class="text6">Sincerely,</p>
              <p class="text6">Dynamic Databse</p>
              
              <p class="text6">support@dynamicdatabase.com</p>
                 </div><!--emailtext-->
                 <div class="last_footer">
                <p><a href="#">copyright © 2016 Dynamic Database | All rights reserved</a></p>
               </div><!--last-footer-->
        </div><!--emaillogoblg-->
				...
				//	echo $message;  
					...
					if(mail($to, $subject, $message, $headers)){
						...
						$callConfig->headerRedirect("login.php?msg=Email sent successfully ");
						...
						$callConfig->headerRedirect("registration.php?msg=fail");
...
//if($sortfield!="" && $order!="") $order=$sortfield.' '.$order;
$whr="status='active'";
$query=$callConfig->selectQuery(TPREFIX.TBL_CONTINENT,'*','',$order,$start,$limit);
