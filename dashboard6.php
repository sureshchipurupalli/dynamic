...
$cfetch = mysql_fetch_object($cres);
  ...
  $mm="select SUM(price) as total, DATE_FORMAT( date_added, '%Y' ) as year from tb_complaints group by DATE_FORMAT(date_added, '%Y') order by DATE_FORMAT(date_added, '%Y') ASC";
 $res5=mysql_query($mm);
$totalprice = mysql_fetch_object($res5);

//$ss="SELECT DATEPART(yyyy,OrderDate) AS OrderYear,DATEPART(mm,date_added) FROM tb_complaints order by ASC";
//$res6=mysql_query($ss);
//$getyear = mysql_fetch_object($res6);

$dd="SELECT  `country_id` FROM `tb_complaints` WHERE  `complaint_category` =".$_GET['cid']." GROUP BY  `country_id`";
 $res3=mysql_query($dd);
 $totalcountries=mysql_num_rows($res3);
 // $totalcountries = count($details);
...
<style>
	.bar-left			{}
	.bar-left>ul		{}
	.bar-left>ul>li		{ width:100%; background:url(images/img3_03.png) no-repeat; margin-bottom:10px;height:35px;}
...
if(id!=""){ 
window.location.href='dashboard6.php?cid='+id; 
}
...
function searchcountry(ss){
	...
	if(sid!=''){
window.location.href='dashboard6.php?cid='+cid+'&sid='+sid+'&countid='+ss;
}else{
	window.location.href='dashboard6.php?cid='+cid+'&countid='+ss;
}
}
</script>
...
<h2 style="padding:25px 25px; text-align:center;font-family:'Open Sans', sans-serif"><?php echo ucfirst($getcategoryname->name); ?>
<?php if($_GET['sid']!=''){ ?>
...
<div class="about-text">
	<div class="mid_wrap">
	<p style="text-align:center;"><?php echo $getcategoryname->description;?></p>
...
<div class="bar">
  ...
  <div class="bar-left">
            ...
            <?php  
			  ...
			  $rt=$_GET['sid'];
			?>      					
				<a href="dashboard6.php?cid=<?php echo $_GET['cid']; ?>&sid=<?php echo $catlist->id; ?>"><li><span><?php echo ucfirst($catlist->department_name); ?></span></li></a>
...
<option value="<?php echo $categories->id;?>" <?php if($_GET['cid']==$categories->id) {?> selected="selected" <?php }?>><?php echo $categories->name;?></option>
<?php } ?>
...
<div class="clear_fix"></div>
            ...
            <?php  
			  ...
			  $rt=$_GET['sid'];
			?>      					
				<a href="dashboard6.php?cid=<?php echo $_GET['cid'];?>&sid=<?php echo $catlist->id; ?>"><li  <?php if($rt==$catlist->id){?> style="width:100%; background:url(images/img31_03.png) no-repeat; margin-bottom:10px; padding:5%;" <?php }?> ><?php echo ucfirst($catlist->department_name); ?></li></a>
             <?php } ?>  
            </ul>
...
<form name="calendrasearch" method="post">
	...
	<input type="text" class="search_dates" placeholder="Enter From date" name="fromdate" id="fromdate" value=<?php echo $_GET['fromdate'];?> ><br>

	<input type="text" class="search_dates" placeholder="Enter To date" name="todate" id="todate" value=<?php echo $_GET['todate'];?>><br>
...
<p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid'];?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid'];?>&status=Closed">Completed Cases: <?php echo $closedcompcnt;?></a> <?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?>,Total Amount:$ <?php echo $fetch->sumprice;?></p>


		<?php }elseif($_GET['sid']=="" && $_GET['countid']!=""){ ?>

<p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&countid=<?php echo $_GET['countid'];?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&countid=<?php echo $_GET['countid'];?>&status=Closed">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?></p>

<?php } elseif($_GET['cid']!="" && $_GET['sid']!='' && $_GET['countid']!="")
{?>
	<p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid']?>&countid=<?php echo $_GET['countid'];?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid']?>&countid=<?php echo $_GET['countid']?>&status=Closed">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?> ,Total Amount:$<?php echo $fetch->sumprice;?></p>
...
{ ?>
	<p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&todate=<?php echo $_GET['todate']?>&fromdate=<?php echo $_GET['fromdate'];?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&todate=<?php echo $_GET['todate']?>&fromdate=<?php echo $_GET['fromdate']?>">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?>,Total Amount:$ <?php echo $fetch->sumprice;?></p>
	
<?php }

elseif($_GET['sid']!='' && $_GET['fromdate']!='' && $_GET['todate']!='')
{ ?>
	<p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid']?>&todate=<?php echo $_GET['todate']?>&fromdate=<?php echo $_GET['fromdate'];?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&sid=<?php echo $_GET['sid']?>&todate=<?php echo $_GET['todate']?>&fromdate=<?php echo $_GET['fromdate']?>">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?>,Total Amount:$ <?php echo $fetch->sumprice;?></p>
...
<?php }
    ...
    <p><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>">Total Posted Cases: <?php echo count($getallresults);?></a>, <?php if($closedcompcnt != 0){?><a href="rankcategory1.php?cid=<?php echo $_GET['cid']?>&status=Closed">Completed Cases: <?php echo $closedcompcnt;?></a><?php }else{?> Completed Cases: <?php echo $closedcompcnt;?> <?php }?>,Total Amount:$ <?php echo $fetch->sumprice;?></p>
...
<select name="searchcat" class="searchcountry" id="searchcat2" onchange="searchcountry(this.value);">
...
<?php }?>
 ...
 <script type="text/javascript">
                ...
                for(var i=0;i<document.getElementById('searchcat2').length;i++)

                {

						if(document.getElementById('searchcat2').options[i].value=="<?php echo $_GET['countid']; ?>")

						{

						document.getElementById('searchcat2').options[i].selected=true
  ...
  window.onload = function () {
    ...
    var chart = new CanvasJS.Chart("chartContainer",
    {
        ...
        data: [
      {
        ...
        { x: new Date(2013, 01, 1), y: 3800 },
        { x: new Date(2014, 02, 1), y: 1500 },
        { x: new Date(2015, 03, 1), y: 2500 },
        { x: new Date(2016, 04, 1), y: 4100 }
	...
	jQuery('#btn2').click(function() {        
		 ...
		 jQuery('#container2').hide();
	         ...
	         <div class="total">
         <?php include("includes/chart/chart50.php");?>
            ...
            <ul>
            	<li>
                	<div class="t-left">
						...
						<form>
						<input type="button" id="btn1" value="days">
						<input type="button" id="btn2" value="months">
						</form>
						
                        <!--<img src="images/dashboard_09.png" />-->
						<!--<div id="chartContainer" style="height: 150px; width: 130%; margin-top:30px"></div>-->
						 <div id="container" style="min-width: 310px; height: 250px; margin: 0 auto" ></div>
                         <div id="container2" style=" height: 250px; margin: 0 auto; display:none"></div>
                    </div>
                    <div class="t-right">
		 ...
		 <div class="percentage">
            ...
            <!--<img src="images/percentage_03.png" />-->
			 ...
			 <div id="piechart" style="width: 250px; height: 200px;padding-top: 32px;"></div>
        ...
        <div class="user">
            ...
            <h3>Total <?php echo $totalcountries ;?> countries</h3>
            <img src="images/man_09.png" />
            <h4><?php echo $cfetch->totalusers;?></h4>
			<?php if($cfetch->totalusers <=1)
			{ ?>
            <p>User raised complaints</p>
			<?php }else{?>
...
document.getElementById('piechart').innerHTML="No Complaints";
